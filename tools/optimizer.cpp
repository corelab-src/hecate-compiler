...
void registerHecatePipeline(cl::opt<std::string> &outputFilename) {
  ...
  PassPipelineRegistration<>(
      "packed_unroll_flex_loop", "Perform Loop Optimization on HE programs",
      [&](OpPassManager &pm) {
        std::string dir;
        std::string stem;
        if (outputFilename != "-") {
  ...
  PassPipelineRegistration<>(
      "dacapo_rh", "Perform automatic bootstrapping placement with Rescale Hoisting",
        [&](OpPassManager &pm) {
          std::string dir;
          std::string stem;
          if (outputFilename != "-") {
  ...
  PassPipelineRegistration<>(
      "pars_rh", "Perform Scale Management on bootstrapped models with Rescale Hoisting",
        [&](OpPassManager &pm) {
          std::string dir;
          std::string stem;
          if (outputFilename != "-") {
