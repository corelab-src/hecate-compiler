...
from hetorch.blas import roll, cint, fint, print_shift_mapping, masking_ciphertext, full_replicate_ciphertext, replicate_ciphertext
...
def bsgsCP_orion(input, weight, slot_length, bsgs_ratio=orion_bsgs_ratio) :
    ...
    weight = F.pad(weight, (0, new_inter_dim - inter_dim, 0, new_out_dim - out_dim), mode = 'constant', value = 0) # (out_dim, expand*out_dim)
    out_dim, inter_dim = weight.shape
    ...
    if is_out_dim_bigger :
        ...
        if not (inter_dim > slot_length/2) :
            ...
            assert inter_dim == slot_length, f"inter_dim: {inter_dim}, slot_length: {slot_length}"
    ...
    if is_out_dim_bigger:
        ...
        if inter_dim % out_dim != 0:
            expand = 1
            weight = weight.T
            weight = F.pad(weight, (0, inter_dim - out_dim), mode = 'constant', value = 0) # (out_dim, expand*out_dim)
        else:
    ...
    baby_step, giant_step, groups, best_shift_mapping, baby_step_list = orion_cal_bsgs_list(cal_bsgs, shift_list, bsgs_ratio)
    print(f"After orion_cal_bsgs_list - baby_step: {baby_step}, giant_step: {giant_step}")
...
def bsgsCP_orion_core(input, weight, slot_length, bsgs_ratio=orion_bsgs_ratio) :
    ...
    baby_step, giant_step, groups, best_shift_mapping, baby_step_list = orion_cal_bsgs_list(out_dim, shift_list, bsgs_ratio)
    print(f"After orion_cal_bsgs_list - baby_step: {baby_step}, giant_step: {giant_step}")
    print_shift_mapping(best_shift_mapping)
                    ...
                    for kh in range(ker_h):
                        for kw in range(ker_w):
                            ...
                            if 0 <= ih < input_h and 0 <= iw < input_w:
                                ...
                                toeplitz_tensor[row_idx, col_idx] = kernel[oc, ic, kh, kw]
...
def orion_conv2d(input_vector, kernel, stride, padding, input_shape):
    ...
    toeplitz_matrix, output_shape = create_toeplitz_matrix(kernel, in_h, in_w, stride, padding)
    output_vector = conv_gemv_cp(input_vector, toeplitz_matrix, N)
        
    return output_vector, output_shape

def create_multiplexed_toeplitz_for_bsgs(kernel, input_h, input_w, stride, padding, input_gap, output_gap):
    out_c, in_c, ker_h, ker_w = kernel.shape
    ...
    print(f"Multiplexed Toeplitz - Input gap: {iG}, Output gap: {oG}, stride: {stride}, padding: {padding}")
    print(f"Clear shapes: in=({in_c}, {input_h}, {input_w}), out=({out_c}, {out_h}, {out_w})")
    print(f"FHE shapes: in=({on_Ci}, {on_Hi}, {on_Wi}), out=({on_Co}, {on_Ho}, {on_Wo})")
...
def orion_conv2d_with_gap(input_vector, kernel, stride, padding, input_shape, 
                          input_gap=1, output_gap=None):
    in_c, in_h, in_w = input_shape
...
def orion_conv2d_cached(input_vector, kernel, stride, padding, input_shape, cache_name, cache_dir, is_caching=False):
    in_c, in_h, in_w = input_shape
    out_c, in_c, ker_h, ker_w = kernel.shape
    try:
...
def orion_conv2d_cached_with_gap(input_vector, kernel, stride, padding, input_shape, cache_name, cache_dir, is_caching=False, input_gap=1, output_gap=None):
    in_c, in_h, in_w = input_shape
    if output_gap is None:
        output_gap = input_gap * stride
    print(f"Multiplexed Toeplitz - Input gap: {input_gap}, Output gap: {output_gap}, stride: {stride}, padding: {padding}")
...
def create_shortcut_mask(input_shape, output_shape, input_gap, output_gap, stride):
    ...
    assert expected_h == out_h and expected_w == out_w, \
...
def orion_shortcut_conv2d_with_gap(input_vector, shortcut_module, input_shape, 
                                    input_gap, output_gap, cache_name, cache_dir, is_caching=False):
    """
    ...
    if isinstance(shortcut_module, nn.Sequential) and len(shortcut_module) == 0:
    ...
    elif hasattr(shortcut_module, '__class__') and shortcut_module.__class__.__name__ == 'LambdaLayer':
        ...
        out_vector, out_shape, current_gap = orion_conv2d_cached_with_gap(
            input_vector, conv_weight, stride, 0, input_shape,
            cache_name=cache_name, cache_dir=cache_dir, is_caching=is_caching,
            input_gap=input_gap, output_gap=output_gap
        )
        ...
        out_vector, out_shape, current_gap = orion_conv2d_cached_with_gap(
            input_vector, conv_layer.weight.detach(), conv_layer.stride[0],
            conv_layer.padding[0], input_shape, 
            cache_name=cache_name, cache_dir=cache_dir, is_caching=is_caching,
            input_gap=input_gap, output_gap=output_gap
        )
...
def orion_avgpool_global(input_vector, input_shape, input_gap, output_gap=32, cache_name=None, cache_dir=None, is_caching=False):
    in_c, in_h, in_w = input_shape
    ...
    result, out_shape, output_gap = orion_avgpool_global_cached(
        input_vector, avg_weight, stride, padding, input_shape,
        cache_name=cache_name, cache_dir=cache_dir, is_caching=False,
        input_gap=input_gap, output_gap=output_gap  # Gap resets to 1 for global pooling
    )
...
def orion_avgpool_global_cached(input_vector, kernel, stride, padding, input_shape, cache_name, cache_dir, is_caching=False, input_gap=1, output_gap=None):
    in_c, in_h, in_w = input_shape
    if output_gap is None:
        output_gap = input_gap * stride
    print(f"Multiplexed Toeplitz - Input gap: {input_gap}, Output gap: {output_gap}, stride: {stride}, padding: {padding}")
...
def orion_avgpool2d_with_gap(input_vector, kernel_size, stride, padding, input_shape, 
                              input_gap=1, output_gap=None, cache_name=None, 
                              cache_dir=None, is_caching=False):
    """
    ...
    kernel = torch.ones(1, 1, kernel_size, kernel_size, dtype=torch.double) / (kernel_size * kernel_size)
    ...
    print(f"AvgPool2d - Input shape: {input_shape}, kernel_size: {kernel_size}, stride: {stride}")
...
def orion_avgpool_cached_with_gap(input_vector, kernel, stride, padding, input_shape, 
                                   cache_name, cache_dir, is_caching=False, 
                                   input_gap=1, output_gap=None):
    in_c, in_h, in_w = input_shape
    ...
    print(f"Multiplexed AvgPool Toeplitz - Input gap: {input_gap}, Output gap: {output_gap}")
...
# ------------------------------------------------------------------
def orion_conv2d_cached_with_gap_multi_cipher(input_vector, kernel, stride, padding, input_shape, 
                                              cache_name, cache_dir, is_caching=False,
                                              input_gap=1, output_gap=None, num_input_ciphers=1):
    in_c, in_h, in_w = input_shape
    if output_gap is None:
        output_gap = input_gap * stride
    ...
    if cache_file.exists() and is_caching:
        ...
        print(f"Output shape: {output_shape}, Block sizes: ({num_row_blocks}, {num_col_blocks})")
        ...
        print(f"Toeplitz shape: ({num_rows}, {num_cols}), num_row_blocks: {num_row_blocks}, num_col_blocks: {num_col_blocks}")
    ...
    if num_row_blocks == 1 and num_col_blocks == 1:
        ...
        if num_input_ciphers > 1:
            ...
            out, out_shape, output_gap = orion_conv2d_cached_with_gap(combined_input, kernel, stride, padding, input_shape, cache_name, cache_dir, True, input_gap, output_gap)
            return out, out_shape, output_gap, 1
        else:
            out, out_shape, output_gap = orion_conv2d_cached_with_gap(input_vector, kernel, stride, padding, input_shape, cache_name, cache_dir, True, input_gap, output_gap)
            return out, out_shape, output_gap, 1

    print(f"== Using multi-cipher convolution == Blocks: ({num_row_blocks}, {num_col_blocks})")
    input_blocks = np.full((num_col_blocks,), Empty(), dtype=object)
    if num_input_ciphers == 1:
        if num_input_ciphers != num_col_blocks:
            # Safe check
            raise ValueError(f"Input cipher count ({num_input_ciphers}) doesn't match column blocks ({num_col_blocks})")
...
def orion_bn_with_gap_multi_cipher(input, bn_layer, output_shape, gap, num_ciphers, scale_factor=32):
    if num_ciphers == 1:
...
def orion_avgpool2d_cached_with_gap_multi_cipher(input_vector, kernel_size, stride, padding, 
                                                 input_shape, cache_name, cache_dir, is_caching=False,
                                                 input_gap=1, output_gap=None, num_input_ciphers=1):
    in_c, in_h, in_w = input_shape
    if output_gap is None:
        output_gap = input_gap * stride
    ...
    print(f"Input gap: {input_gap}, Output gap: {output_gap}, Num input ciphers: {num_input_ciphers}")
    ...
    print(f"Using multi-cipher pooling computation == Blocks: ({num_row_blocks}, {num_col_blocks})")
...
def orion_avgpool2d_preboot(input_vector, kernel_size, stride, padding, 
                                                 input_shape, cache_name, cache_dir, is_caching=False,
                                                 input_gap=1, output_gap=None, num_input_ciphers=1):
    in_c, in_h, in_w = input_shape
    if output_gap is None:
...
def prepare_fc_weight_for_multiplexed_input(fc_weight, input_shape, input_gap):
    ...
    assert in_features == in_c * in_h * in_w, f"Dimension mismatch: {in_features} != {in_c * in_h * in_w}"
        ...
        for c in range(in_c):
            for h in range(in_h):
                for w in range(in_w):
                    ...
                    mpx_idx = mpx_channel * (mpx_h * mpx_w) + mpx_h_pos * mpx_w + mpx_w_pos
...
def orion_preprocess(input, scale_factor=32):
    ...
    input_flattened = F.pad(input_flattened, (0, 2**15 - input_flattened.shape[0]), mode='constant', value=0)
...
def orion_multiplexed_postprocess(result_tensor, expected_shape, gap=1, scale_factor=32):
    if gap == 1:
