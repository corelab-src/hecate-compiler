...
def linear_generate_random_data(OUT_DIM: int, INTER_DIM: int, IN_DIM: int, VALUE_RANGE: float):
    weight = torch.empty(OUT_DIM, INTER_DIM, dtype=torch.double).uniform_(-VALUE_RANGE, VALUE_RANGE)
    bias   = torch.zeros(OUT_DIM, IN_DIM, dtype=torch.double)
    input  = torch.empty(INTER_DIM, IN_DIM, dtype=torch.double).uniform_(-VALUE_RANGE, VALUE_RANGE)
...
def generate_random_data(IN_DIM: int, INTER_DIM: int, OUT_DIM: int, VALUE_RANGE: float):
    input  = torch.empty(IN_DIM, INTER_DIM, dtype=torch.double).uniform_(-VALUE_RANGE, VALUE_RANGE)
    weight = torch.empty(INTER_DIM, OUT_DIM, dtype=torch.double).uniform_(-VALUE_RANGE, VALUE_RANGE)
