...
def full_replicate_ciphertext(ctxt, input_length, slot_length):
    ...
    result, packing_size, packing_times = replicate_ciphertext(
        ctxt, input_length, packing_times
    )
...
def replicate_ciphertext(result, input_length, repeat_times):
    ...
    if res_power_of_2 == 1:
        ...
        if res3_power_of_2 == 0:  # repeat_times = 3 * (2 ** k)
            result[0] = (
                result[0]
                + result[0].rotate(input_length)
                + result[0].rotate(2 * input_length)
            )
    ...
    for i in range(giant_step):
        ...
        if torch.any(padded_weight[offset : offset + baby_step, :] != 0):
            for j in range(baby_step):
                ...
                if i != 0 and j == 0:
                    if (
                        not isinstance(result[0], Empty)
                        and (offset - group_offset) != 0
                    ):
...
def bsgsCP_with_zeros(input, weight, slot_length):
    ...
    print(
        f"bsgsCP_with_zeros inter_dim: {inter_dim}, baby_step: {baby_step}, giant_step: {giant_step}"
    )

    weight = torch.stack([torch.roll(weight[i, :], -i) for i in range(out_dim)])
    weight = weight.T
    padded_weight = torch.zeros(
        (inter_dim, baby_step * (giant_step - 1) + out_dim), dtype=torch.double
    )
    ...
    padded_weight = F.pad(
        padded_weight,
        (0, slot_length - padded_weight.shape[1]),
        mode="constant",
        value=0,
    )
...
def diagCP(input, weight, slot_length):
    ...
    if out_dim > inter_dim:
        ...
        input, packing_size, packing_times = replicate_ciphertext(
            input, out_dim, div_times
        )
        weight = F.pad(weight, (0, out_dim - weight.shape[1]), mode="constant", value=0)
        print(
            f"out_dim > inter_dim, reshape weight.shape: {out_dim} > {inter_dim}, {weight.shape}"
        )
        out_dim, inter_dim = weight.shape
    else:
        input[0] = input[0] + roll(input[0], inter_dim)  # shape (slot_length,)
