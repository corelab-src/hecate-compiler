    ...
    def polynomial(x):
        ...
        for i, j, deg, leaf in calc_order:
            if deg == 0:
                ...
                tmpPoly[(i, j)] = (
                    tmpPoly[(i + 1, 2 * j + 1)] * giantTs[deg] + tmpPoly[(i + 1, 2 * j)]
                )
...
def shapeClosure(
    nt, bb, fh, fw, s, hi, ho, wi, wo, ni, no, ci, co, ki, ko, ti, to, pi, po, q
):
    ...
    def ParMultWgt(U):
        U = F.pad(
            U,
            (0, 0, 0, 0, 0, ki * ki * ti - ci, 0, q * pi - co),
            mode="constant",
            value=0,
        )
        U = einops.repeat(
            U,
            "(q pi) (ti s1 s2) fh fw -> (fh fw q pi) (ti h s1 w s2)",
            q=q,
            pi=pi,
            s1=ki,
            s2=ki,
        ...
        U = einops.rearrange(
            U,
            "(fh fw q pi) (ni A) -> ni q fh fw (pi A)",
            q=q,
            pi=pi,
            fh=fh,
            fw=fw,
        ...
        M = F.pad(
            M,
            (((fw - 1) // 2), ((fw - 1) // 2), ((fh - 1) // 2), ((fh - 1) // 2)),
            mode="constant",
            value=0,
        )
        M = torch.stack(
            [M[i : i + hi, j : j + wi] for i in range(fh) for j in range(fw)]
        )
        M = einops.repeat(
            M,
            "(fh fw) h w -> ni q fh fw (kk h s1 w s2)",
            fh=fh,
            fw=fw,
            q=q,
            kk=nt // (hi * ki * wi * ki),
    ...
    def DwMultWgt(U):
        ...
        U = einops.repeat(
            U,
            "(ti s1 s2) i fh fw -> fh (fw i) (ti h s1 w s2)",
            s1=ki,
            s2=ki,
            h=hi,
            w=wi,
        ...
        U = einops.repeat(
            U, "fh fw (ni A) -> ni fh fw (pi A)", pi=pi, fh=fh, fw=fw, ni=ni
        )
        M = torch.ones(hi, wi, dtype=torch.double)
        M = F.pad(
            M,
        ...
        M = einops.repeat(
            M,
            "(fh fw) h w -> ni fh fw (kk h s1 w s2)",
            fh=fh,
            fw=fw,
            kk=nt // (hi * ki * wi * ki),
            s1=ki,
    ...
    def ParBNConst(C):
        ...
        C = einops.repeat(
            C, "(to s1 s2) -> (to ho s1 wo s2)", to=to, s1=ko, s2=ko, ho=ho, wo=wo
        )
    ...
    def ParInBNConst(C):
        ...
        C = einops.repeat(
            C, "(ti s1 s2) -> (ti hi s1 wi s2)", ti=ti, s1=ki, s2=ki, hi=hi, wi=wi
        )
    ...
    def DownSelecting():
        ...
        S = einops.repeat(
            S,
            " (ki1 ti1) (ki2 ti2) -> ki1 ti1 ti2 hi s1 ki2 wi s2 ki",
            ki1=ki,
            ti1=ti,
            ki2=ki,
            ti2=ti,
        ...
        S = einops.rearrange(
            S, "ki1 ti1 ti2 hi s1 ki2 wi s2 ki -> ki1 ti1 ti2 (hi s1) ki2 (wi s2) ki"
        )
        ...
        S = einops.rearrange(
            S, "ki1 ti1 ti2 hi ki2 wi ki -> ki1 ti1 (ti2 hi ki2 wi ki)"
        )
        ...
        for i1 in range(ki):  # ky
            for i2 in range(ti):
                ...
                C[i8] = C[i8] + roll(
                    A[i7] * S[i7, i1, i2, :],
                    -(
                        (ki * ki * hi * wi * (i2 - i5) + ki * wi * (i1 - i3) - ki * i4)
                        + (i8 - i7) * nt
                    ),
                )
        for i in range(no):
    ...
    def MaxPoolPad(A):
        ...
        for ii in range(ni):
            ...
            for j in range(fw):
                B[ii] = maximum(
                    B[ii], roll(B[ii], -(j - (fw - 1) // 2) * ki) * M[1, j] + D[1, j]
                )
            for j in range(fh):
                B[ii] = maximum(
                    B[ii],
                    roll(B[ii], -(j - (fh - 1) // 2) * ki * ki * wi) * M[j, 1]
                    + D[j, 1],
                )
...
def Reshape(W, shape):
    ...
    W = einops.rearrange(
        W,
        "outdim (to ko1 ko2 ho wo) -> outdim (to ho ko1 wo ko2)",
        outdim=W.shape[0],
        to=to,
        ko2=ko,
        ko1=ko,
...
def InferShapes(shapes):
    ...
    shapes["ni"] = cint(
        (shapes["ki"] * shapes["ki"] * shapes["hi"] * shapes["wi"] * shapes["ti"])
        / shapes["nt"]
    )
    shapes["no"] = cint(
        (shapes["ko"] * shapes["ko"] * shapes["ho"] * shapes["wo"] * shapes["to"])
        / shapes["nt"]
    )
    shapes["pi"] = pow(
        2,
...
def CascadeConcat(shapes1, shapes2):
    shapes = shapes1.copy()
    if (shapes1["co"] != shapes2["co"]) or (
        shapes1["co"] % (shapes["ko"] * shapes["ko"]) != 0
    ):
