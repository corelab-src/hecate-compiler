...
def hc_parser(command_path):
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="Format",
        help="Benchmark name, default is Format",
    )
    # Optional arguments
    ...
    if not isinstance(args.library, str):
        parser.error(
            "Error: Library is not a string, Choices are: HEAAN, SEAL, HEONGPU"
        )
    else:
    ...
    if args.input.lower() == "default setup":
        ...
        print(
            f"If there is no input data in {args.benchmark} file, use (4096, -1, 1) random data"
        )
