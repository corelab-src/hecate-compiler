...
def python_reference_pca(x, y, cov, n_components, epochs):
    def power_iteration(cov, epochs):
        ...
        for _ in range(epochs+1):
            ...
            print(f"inv_input: {b_k1_squared}, sqrt_tmp: {b_k1_norm_i}, inv_sqrt_result: {b_k1_norm}")
            b_k = b_k1 * b_k1_norm
        return b_k
    ...
    for i in range(n_components):
        ...
        print(f"max(inv_input): {np.max(inv_input)}, min(inv_input): {np.min(inv_input)}")
...
if __name__ == "__main__":
    ...
    rms = np.sqrt(np.mean([np.power(abs(result_tensor[i]-ref_tensor[i]), 2) for i in range(4)]))
