...
def getModel():
    ...
    model_dict = torch.load(
        str(hecate_dir) + "/examples/data/mobileNet_silu_model",
...
def MobileNet(ctxt):
    ...
    out = HE_ConvBN(
        close, input_var, model.module.pre_layer.Conv2d, model.module.pre_layer.bn
    )
    ...
    for i in range(0, len(model.module.Depthwise)):
        ...
        out = HE_DwConv(
            close, out, model.module.Depthwise[i].dwConv2d, model.module.Depthwise[i].bn
        )
        out[0] = hc.bootstrap(out[0])
        out = act(out)

        inconv_0_pointwise_shapes = CascadeConv(
            inconv_0_shapes, model.module.Depthwise[i].pointwiseConv2d.Conv2d
        )
        close = shapeClosure(**inconv_0_pointwise_shapes)
        out = HE_ConvBN(
            close,
            out,
            model.module.Depthwise[i].pointwiseConv2d.Conv2d,
            model.module.Depthwise[i].pointwiseConv2d.bn,
        )
        out[0] = hc.bootstrap(out[0])
        out = act(out)
