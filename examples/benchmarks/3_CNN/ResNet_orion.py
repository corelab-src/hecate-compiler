...
def ResNet_orion(*ctxt):
    ...
    mpp_vec[0] = ctxt[0]
    conv1 = model.conv1
    out, out_shape = ht.orion_conv2d_cached(
        mpp_vec,
        conv1.weight.detach(),
        conv1.stride[0],
        conv1.padding[0],
    ...
    for i in range(0, len(model.layer1)):
        ...
        out, out_shape = ht.orion_conv2d_cached(
            out,
            layer_conv1.weight.detach(),
            layer_conv1.stride[0],
            layer_conv1.padding[0],
            out_shape,
            f"layer1_{i}_conv1",
            cache_dir,
            is_caching=True,
        ...
        out, out_shape = ht.orion_conv2d_cached(
            out,
            layer_conv2.weight.detach(),
            layer_conv2.stride[0],
            layer_conv2.padding[0],
            out_shape,
            f"layer1_{i}_conv2",
            cache_dir,
            is_caching=True,
    ...
    for i in range(len(model.layer2)):
        ...
        out, out_shape, current_gap = ht.orion_conv2d_cached_with_gap(
            ...
            layer_conv1.padding[0],
            out_shape,
            cache_name=f"layer2_{i}_conv1",
            cache_dir=cache_dir,
            is_caching=True,
            input_gap=current_gap,
            output_gap=2,
        )
        out = bootstrap(out)
        ...
        if i == 0:
            print(
                f"Applying shortcut: input shape {residual_shape} (gap={residual_gap}) -> output shape {out_shape} (gap={current_gap})"
            )
            shortcut_module = model.layer2[i].shortcut
            shortcut_out, shortcut_shape, shortcut_gap = (
                ht.orion_shortcut_conv2d_with_gap(
                    shortcut_out,
                    shortcut_module,
                    residual_shape,
                    input_gap=residual_gap,
                    output_gap=current_gap,
                    cache_name=f"layer2_{i}_shortcut",
                    cache_dir=cache_dir,
                    is_caching=True,
                )
            )
        ...
        if i == 0:
            ...
            shortcut_out, shortcut_shape, shortcut_gap = (
                ht.orion_shortcut_conv2d_with_gap(
                    ...
                    input_gap=residual_gap,
                    output_gap=current_gap,
                    cache_name=f"layer3_{i}_shortcut",
                    cache_dir=cache_dir,
                    is_caching=True,
                )
            )
    ...
    print("avgpool")
    out, out_shape, current_gap = ht.orion_avgpool_global(
        out, out_shape, current_gap, 32, "layer4_avgpool", cache_dir, is_caching=True
    )
