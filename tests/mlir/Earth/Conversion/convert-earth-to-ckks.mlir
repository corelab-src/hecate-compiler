...
// CHECK-SAME:   %[[ARG0:.*]]: tensor<1x!ckks.poly<2 * 15>>,
// CHECK-SAME:   %[[ARG1:.*]]: tensor<1x!ckks.poly<2 * 15>>
// CHECK-SAME: ) -> tensor<1x!ckks.poly<2 * 15>> {
// CHECK:   %[[DST:.*]] = tensor.empty() : tensor<1x!ckks.poly<2 * 15>>
// CHECK:   %[[RESULT:.*]] = "ckks.addcc"(%[[DST]], %[[ARG0]], %[[ARG1]]) : (tensor<1x!ckks.poly<2 * 15>>, tensor<1x!ckks.poly<2 * 15>>, tensor<1x!ckks.poly<2 * 15>>) -> tensor<1x!ckks.poly<2 * 15>>
// CHECK:   return %[[RESULT]] : tensor<1x!ckks.poly<2 * 15>>
// CHECK: }
